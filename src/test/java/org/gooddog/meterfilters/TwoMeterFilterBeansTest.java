...
@SpringBootTest
@Import({MeterGenerator.class, MeterRegistryConfig.class, TwoMeterFilterBeansConfiguration.class})
@ActiveProfiles({"properties-filter-accept-all", "two-meter-filter-beans"})
@Slf4j
public class TwoMeterFilterBeansTest {

    @Autowired
    private MeterRegistry meterRegistry;

    /* The two filters defined in the configuration */
    @Autowired
    private MeterFilter filter1;

    @Autowired
    private MeterFilter filter2;
